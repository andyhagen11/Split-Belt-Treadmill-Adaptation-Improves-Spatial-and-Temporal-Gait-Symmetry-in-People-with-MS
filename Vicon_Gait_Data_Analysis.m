...
%% Step Length Asymmetry (total, positional contribution, timing contribution, and velocity contribution) , Step Position Asymmetry, Step Time Asymmetry
    % Contribution analysis based on temporal and spatial decomposition of SLA detailed in:
...
% Created a copy of L & R heel strike matrices as lhs and rhs so it is not affected for other analysis
Rhs = R_HeelStrikes;
Lhs = L_HeelStrikes;
Rto = R_ToeOffs;
Lto = L_ToeOffs;
...
Xs_SHS1 = zeros(1,Num_Strikes(~Fast_Leg)-1); % Slow leg ankle-hip at slow leg heelstrike 1
Xf_SHS1 = Xs_SHS1; % Fast leg ankle-hip at slow leg heelstrike 1
Xs_SHS2 = Xs_SHS1; % Slow leg ankle-hip at slow leg heelstrike 2 (SHS1 shifted by one position)
Xf_SHS2 = Xs_SHS1; % Fast leg ankle-hip at slow leg heelstrike 2 (SHS1 shifted by one position)
...
% Limb Excursion - Modified stride length - Distance covered from toe off to heel strike on same leg. On a treadmill true stride length will be 0
